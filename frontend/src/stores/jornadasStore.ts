/**
 * Store de estadísticas agregadas del módulo Jornadas (GET /jornadas/estadisticas).
 *
 ...
 *   - Con datos frescos (< JORNADAS_TTL_MS): no toca la red, retorna al toque.
 *   - Con datos viejos (>= JORNADAS_TTL_MS): NO setea loading, revalida en
...
interface JornadasState {
  data: EstadisticasJornadas | null;
...
const initialState: JornadasState = {
  data: null,
  loading: false,
  error: null,
  revalidating: false,
...
function createJornadasStore() {
  const store = writable<JornadasState>(initialState);
  ...
  return {
    ...
    load: async (opts: { force?: boolean } = {}) => {
      ...
      const isFresh = hasLoadedOnce && Date.now() - current.lastFetchedAt! < JORNADAS_TTL_MS;
      ...
      try {
        const data = await getEstadisticasJornadas();
...
export const jornadasStore = createJornadasStore();
