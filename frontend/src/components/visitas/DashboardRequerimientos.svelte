<script lang="ts">
  ...
  $: filteredReqs = allReqs.filter((r) => {
    ...
    const byCG =
      !filterCentroGestor || r.centros_gestores.includes(filterCentroGestor);
    const byCom =
      !filterComuna || r.solicitante.comuna_corregimiento === filterComuna;
    return inPeriod && byCG && byCom;
  });

  $: filteredVisitas = allVisitas.filter(
    (v) => new Date(v.created_at) >= startOfPeriod(periodo),
  );

  // ── KPIs ─────────────────────────────────────────────────────────────────
  $: totalReqs = filteredReqs.length;
  $: totalVisitas = filteredVisitas.length;
  $: resueltos = filteredReqs.filter(
  ...
  $: avgAvance =
    totalReqs > 0
      ? Math.round(
          filteredReqs.reduce((s, r) => s + r.porcentaje_avance, 0) / totalReqs,
        )
      : 0;
  $: tasaResolucion =
    totalReqs > 0 ? Math.round((resueltos / totalReqs) * 100) : 0;
  ...
  $: byEstado = ESTADOS.map((e) => ({
    ...e,
    count: filteredReqs.filter((r) => r.estado === e.id).length,
  }));
  ...
  $: byPrioridad = PRIORIDADES.map((p) => ({
    ...p,
    count: filteredReqs.filter((r) => r.prioridad === p.id).length,
  }));
  ...
  $: tablaComunas = (() => {
    const map: Record<
      string,
      { barrios: Set<string>; cgs: Set<string>; visitas: number; reqs: number }
    > = {};
    for (const r of filteredReqs) {
      const com = r.solicitante.comuna_corregimiento || "Sin comuna";
      if (!map[com])
        map[com] = { barrios: new Set(), cgs: new Set(), visitas: 0, reqs: 0 };
      if (r.solicitante.barrio_vereda)
        map[com].barrios.add(r.solicitante.barrio_vereda);
    ...
    return Object.entries(map)
      .map(([com, d]) => ({
        comuna: com,
        barrios: d.barrios.size,
        centrosGestores: d.cgs.size,
        visitas: Math.min(d.visitas, filteredVisitas.length),
        reqs: d.reqs,
      }))
...
<div class="db-root">
      ...
      <div class="period-pills">
        {#each periodos as p}
          <button
            ...
            on:click={() => (periodo = p.id)}>{p.label}</button
          >
        {/each}
      </div>
      <!-- Filters -->
          ...
          {#if e.count > 0 || totalReqs === 0}
            <div class="hbar-item">
              ...
              <div class="hbar-track">
                <div
                  class="hbar-fill"
                  style="width: {totalReqs > 0
                    ? Math.round((e.count / maxEstado) * 100)
                    : 0}%; background: {e.color};"
                ></div>
              </div>
      ...
      <div class="prio-grid">
        {#each byPrioridad as p}
          <div class="prio-cell" style="border-color: {p.color}">
            ...
            {#if totalReqs > 0}
              <span class="prio-pct"
                >{Math.round((p.count / totalReqs) * 100)}%</span
              >
            {/if}
          </div>
        {/each}
      ...
      {#if totalReqs > 0}
        <div class="donut-wrap">
          <div
            ...
            {byPrioridad[0].color} 0% {(byPrioridad[0].count / totalReqs) *
              100}%,
            {byPrioridad[1].color} {(byPrioridad[0].count / totalReqs) *
              100}% {((byPrioridad[0].count + byPrioridad[1].count) /
              totalReqs) *
              100}%,
            {byPrioridad[2].color} {((byPrioridad[0].count +
              byPrioridad[1].count) /
              totalReqs) *
              100}% {((byPrioridad[0].count +
              byPrioridad[1].count +
              byPrioridad[2].count) /
              totalReqs) *
              100}%,
            {byPrioridad[3].color} {((byPrioridad[0].count +
              byPrioridad[1].count +
              byPrioridad[2].count) /
              totalReqs) *
              100}% 100%
    ...
    {:else}
      <div class="hbar-list">
        {#each byCentroGestor as cg}
          <div class="hbar-item">
            <span class="hbar-label hbar-label-wide" title={cg.cg}
              >{cg.cg.length > 35 ? cg.cg.slice(0, 32) + "…" : cg.cg}</span
            >
                ...
                <td>
                  <div class="mini-bar-table">
                    <div
                      class="mini-bar-fill"
                      style="width: {totalReqs > 0
                        ? Math.round((row.reqs / totalReqs) * 100)
                        : 0}%"
                    ></div>
                  </div>
                  <span class="trw-pct"
  ...
  <div class="db-footer">
    <span
      >Alcaldía de Santiago de Cali — Sistema de Seguimiento de Requerimientos</span
    >
    <span
