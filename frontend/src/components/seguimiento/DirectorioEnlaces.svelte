<script lang="ts">
  ...
  /* Derive unique centros gestores from API data */
  $: centrosGestores = (() => {
    const seen = new Map<string, CentroGestorDynamic>();
    let colorIdx = 0;
    for (const c of contactos) {
      const key = c.centro_gestor.trim();
      if (!seen.has(key)) {
        ...
        seen.set(key, {
          ...
          nombre: key,
          sigla,
          color: CG_PALETTE[colorIdx % CG_PALETTE.length],
        });
        colorIdx++;
      }
    }
  ...
  /* Map API contacts → EnlaceView for display */
  $: enlaces = contactos.map((c): EnlaceView => {
    const cgKey = c.centro_gestor.trim();
    const cg = centrosGestores.find((g) => g.nombre === cgKey);
    return {
      id: String(c.id),
      ...
      cargo: c.funcion || "",
      cedula: c.cedula || "",
      centro_gestor_id: cg?.id || "",
      centro_gestor_nombre: cgKey,
      activo: true,
    };
  });
  ...
  let searchText = "";
  let selectedEnlace: EnlaceView | null = null;

  // Build per-centro-gestor stats
  $: centroStats = (() => {
    const stats: Record<
      string,
      { total: number; activos: number; conReqs: number; totalReqs: number }
    > = {};
    for (const cg of centrosGestores) {
      const cgEnlaces = enlaces.filter((e) => e.centro_gestor_id === cg.id);
      const cgEnlaceIds = new Set(cgEnlaces.map((e) => e.id));
      const reqsAsociados = requerimientos.filter(
        (r) => r.enlace_id && cgEnlaceIds.has(r.enlace_id),
      );
  ...
  $: filteredEnlaces = (() => {
    ...
    if (filtroActivo === "con-req") {
      const enlaceIdsConReq = new Set(
        requerimientos.filter((r) => r.enlace_id).map((r) => r.enlace_id),
      );
  ...
  function getCentroColor(centroId: string): string {
    return centrosGestores.find((c) => c.id === centroId)?.color || "#64748b";
  }

  function getCentroSigla(centroId: string): string {
    return centrosGestores.find((c) => c.id === centroId)?.sigla || "?";
  }
  ...
  onMount(async () => {
    try {
      ...
      errorMsg = err?.message || "Error al cargar el directorio de contactos";
    } finally {
      loading = false;
    }
  });
  ...
  <main class="view-body">
    ...
    {:else}
      ...
      <div class="summary-row">
        ...
        <div class="summary-card">
          <span class="sc-num" style="color: #f59e0b"
            >{centrosGestores.length}</span
          >
        ...
        <div class="cg-grid">
          {#each centrosGestores as cg (cg.id)}
            ...
            <button
              ...
              on:click={() => {
                filtroCentroGestor = filtroCentroGestor === cg.id ? "" : cg.id;
              }}
              style="border-left-color: {cg.color}"
            >
              <div class="cg-card-header">
                <span class="cg-sigla" style="background: {cg.color}"
                  >{cg.sigla}</span
                >
              ...
              <div class="cg-card-stats">
                ...
                <div class="cg-stat">
                  <span class="cg-stat-num" style="color: #22c55e"
                    >{stats.activos}</span
                  >
                  <span class="cg-stat-lbl">activos</span>
                </div>
                <div class="cg-stat">
                  <span class="cg-stat-num" style="color: #2563eb"
                    >{stats.conReqs}</span
                  >
                  <span class="cg-stat-lbl">c/req</span>
                </div>
                <div class="cg-stat">
                  <span class="cg-stat-num" style="color: #f59e0b"
                    >{stats.totalReqs}</span
                  >
        ...
        <div class="filters-row">
          ...
          <div class="filter-chips">
            <button
              ...
              on:click={() => (filtroActivo = "todos")}>Todos</button
            >
            <button
              class="fchip"
              class:active={filtroActivo === "con-req"}
              on:click={() => (filtroActivo = "con-req")}>Con Reqs</button
            >
            <button
              class="fchip"
              class:active={filtroActivo === "sin-req"}
              on:click={() => (filtroActivo = "sin-req")}>Sin Reqs</button
            >
          </div>
          {#if filtroCentroGestor}
            <button
              class="clear-filter"
              on:click={() => (filtroCentroGestor = "")}
            >
          ...
          <table class="enlaces-table">
            ...
            <tbody>
              {#each filteredEnlaces as enlace (enlace.id)}
                ...
                <tr
                  class="enlace-row"
                  class:selected-row={selectedEnlace?.id === enlace.id}
                >
                  <td>
                    <span
                      class="cg-badge"
                      style="background: {getCentroColor(
                        enlace.centro_gestor_id,
                      )}">{getCentroSigla(enlace.centro_gestor_id)}</span
                    >
                  </td>
                  <td class="td-nombre">
                  ...
                  <td>
                    <span
                      class="estado-badge"
                      class:activo={enlace.activo}
                      class:inactivo={!enlace.activo}
                    >
                  ...
                  <td>
                    <button
                      ...
                      on:click={() => {
                        selectedEnlace =
                          selectedEnlace?.id === enlace.id ? null : enlace;
                      }}
                    >
                  ...
                  <tr class="detail-row">
                    <td colspan="8">
                      <div class="enlace-detail">
                        <div class="detail-info-grid">
                          ...
                          <div class="dinfo">
                            ...
                            <span class="dinfo-value"
                              >{enlace.centro_gestor_nombre}</span
                            >
                          </div>
                          <div class="dinfo">
                            <span class="dinfo-label">Cédula</span>
                            <span class="dinfo-value"
                              >{enlace.cedula || "N/A"}</span
                            >
                          </div>
                          <div class="dinfo">
                          ...
                          <div class="reqs-mini-list">
                            {#each enlaceReqs as req (req.id)}
                              <div class="req-mini">
                                ...
                                <span
                                  class="rm-estado"
                                  style="background: {getEstadoColor(
                                    req.estado,
                                  )}">{req.estado}</span
                                >
                                <span class="rm-desc"
                                  >{req.descripcion.slice(0, 70)}{req
                                    .descripcion.length > 70
                                    ? "..."
                                    : ""}</span
                                >
                                <span class="rm-avance"
                                  >{req.porcentaje_avance}%</span
                                >
                                <button
                                  class="rm-link"
                                  on:click={() =>
                        ...
                        {:else}
                          <p class="no-reqs-msg">
                            No tiene requerimientos asignados actualmente.
                          </p>
                        {/if}
                      </div>
                    </td>
              ...
              {:else}
                <tr
                  ><td colspan="8" class="empty-table"
                    >No se encontraron enlaces con los filtros seleccionados</td
                  ></tr
                >
              {/each}
            </tbody>
